      ...
      if(dragged){
        if (clicked) {
          ...
          hammer.on('pan pinch', function(ev) { 
            ...
            var coorX = (lastPosX + deltaX)*currentScale;
            var coorY = (lastPosY + deltaY)*currentScale;
            ...
            if (ev.isFinal) {
              ...
              if (coorX<coorMaxX*currentScale) {coorX=coorMaxX}
              if (coorX>coorMinX*currentScale) {coorX=coorMinX}
              if (coorY<coorMaxY*currentScale) {coorY=coorMaxY}
              if (coorY>coorMinY*currentScale) {coorY=coorMinY}
              el.css({
                transform: 'translate3d(' + coorX + 'px, ' + coorY + 'px, 0) scale(' + currentScale + ')'
              });
              lastPosX = coorX;
