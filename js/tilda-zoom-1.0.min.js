      ...
      if(dragged){
        if (clicked) {
          ...
          hammer.on('pan pinch', function(ev) { 
            ...
            var newScale = adjustScale*ev.scale;
            if (newScale>maxScale) {newScale=maxScale}
            if (newScale<minScale) {newScale=minScale}
            var deltaX = ev.deltaX*newScale;
            var deltaY = ev.deltaY*newScale;
            var coorX = (lastPosX + deltaX);
            var coorY = (lastPosY + deltaY);
            console.log(coorX, coorY, newScale);
            el.css({
              transform: 'translate3d(' + coorX + 'px, ' + coorY + 'px, 0) scale(' + newScale + ')'
            });
            if (ev.isFinal) {
              isDragging = false;
