  ...
  $('.zcontainer').each(function(index) {
    ...
    if (allowZoom) {
      ...
      hammer.on('pinch', function(ev) {
        ...
        // coorX = lastPosX*currentScale + (imageW*currentScale-lastPosX*currentScale);
        // coorY = lastPosY*currentScale + (imageH*currentScale-lastPosY*currentScale);
        // coorX = maxW - (imageW*currentScale+lastPosX*currentScale);
        // coorY = maxH - (imageH*currentScale+lastPosY*currentScale);
        if (ev.pointers[0].clientX > ev.pointers[1].clientX) {
          x1 = ev.pointers[0].clientX*currentScale - ev.pointers[1].clientX*currentScale;
        } else {
          x1 = ev.pointers[1].clientX*currentScale - ev.pointers[0].clientX*currentScale;
        }
        if (ev.pointers[0].clientY > ev.pointers[1].clientY) {
          y1 = ev.pointers[0].clientY*currentScale - ev.pointers[1].clientY*currentScale;
        } else {
          y1 = ev.pointers[1].clientY*currentScale - ev.pointers[0].clientY*currentScale;
        }

        // var dist = Math.sqrt((ev.pointers[0].clientX-ev.pointers[1].clientX) * (ev.pointers[0].clientX-ev.pointers[1].clientX) + (ev.pointers[0].clientY-ev.pointers[1].clientY) * (ev.pointers[0].clientY-ev.pointers[1].clientY));

        coorX = lastPosX*currentScale + x1;
        coorY = lastPosY*currentScale + y1;
        // console.log('x', x1)
        // console.log('y', y1)
        // console.log(dist)
        console.log(ev)
        ...
        //   coorX = lastPosX*currentScale -  x1 * currentScale + ((maxW-currentW)/2) * currentScale;
        // }
        // if (maxH>imageH*currentScale) {
        //   coorY = lastPosY*currentScale -  y1 * currentScale + ((maxH-currentH)/2) * currentScale;
        // }
