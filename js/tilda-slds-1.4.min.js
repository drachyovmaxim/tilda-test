...
sliderWrapper.addClass('t-slds_animated');sliderWrapper.css({transform:'translate3d(-'+(sliderWidth*pos)+'px, 0, 0)'});setTimeout(function(){sliderWrapper.removeClass('t-slds_animated');sliderWrapper.attr('data-slider-animated','');if(cycle=='yes'){if(pos==(totalSlides+1)){pos=1};if(pos==0){pos=totalSlides};sliderWrapper.css({transform:'translate3d(-'+(sliderWidth*pos)+'px, 0, 0)'});if(sliderType!==!0){t_slds_ActiveSlide(recid,pos,totalSlides)}
sliderWrapper.attr('data-slider-pos',pos);if(el.find('.t-slds__item-loaded').length<totalSlides+2){t_slds_UpdateImages(recid,pos)}}
if(el.find('.t-slds__item-loaded').length<totalSlides+2){if(window.lazy=='y'){t_lazyload_update()}
t_slds_UpdateImages(recid,pos)}},sliderTransition);t_slds_ActiveBullet(recid,pos,totalSlides);t_slds_ActiveSlide(recid,pos,totalSlides);if(sliderWrapper.attr('data-slider-correct-height')=='true'){t_slds_SliderHeight(recid)}
t_slds_SliderArrowsHeight(recid);t_slds_ActiveCaption(recid,pos,totalSlides);if(el.find('.t-slds__item-loaded').length<totalSlides+2){if(window.lazy=='y'){t_lazyload_update()}
...
function t_slds_ActiveSlide(recid,pos,totalSlides){var el=$('#rec'+recid),slide=el.find('.t-slds__item'),slideActive=el.find('.t-slds__item[data-slide-index="'+pos+'"]');slide.removeClass('t-slds__item_active');if(pos==0){el.find('.t-slds__item[data-slide-index="'+totalSlides+'"]').addClass('t-slds__item_active')}
if(pos==totalSlides+1){el.find('.t-slds__item[data-slide-index="'+1+'"]').addClass('t-slds__item_active')}
